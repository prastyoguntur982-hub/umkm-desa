<!-- Modal Tambah -->
<div id="tambah-strukturOrganisasi" tabindex="-1" aria-hidden="true"
        ...
        <div class="relative bg-white rounded-lg shadow-sm dark:bg-gray-700">
            ...
            <div
                ...
                <button type="button"
                    ...
                    data-modal-hide="tambah-strukturOrganisasi">
            ...
            <div class="max-w-lg mx-auto p-6 bg-white dark:bg-gray-800 rounded-lg shadow-lg">
                <form action="{{ route('admin.struktur-organisasi.store') }}" method="POST"
                    ...
                    <div class="mb-4">
                        <label class="mb-2 block text-sm font-medium text-gray-900 dark:text-white">Jabatan</label>
                        <input type="text" name="jabatan"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:text-white"
                            required>
                    </div>
                    <div class="mb-4">
                        <label class="mb-2 block text-sm font-medium text-gray-900 dark:text-white">NIP</label>
                    ...
                    <div class="mb-4">
                        <label class="mb-2 block text-sm font-medium text-gray-900 dark:text-white">Parent ID</label>
                        <select name="parent_id" @if (count($struktur) > 0) required @endif
                            ...
                            <option value="" disabled selected>
                                {{ count($struktur) > 0 ? 'Pilih Posisi Bagan' : 'Belum ada posisi lain (posisi ketua)' }}
                            </option>

                            @foreach ($struktur as $item)
                                <option value="{{ $item->id }}">Dibawah {{ $item->jabatan }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-4">
                        ...
                        <small class="text-gray-500 dark:text-gray-400">Biarkan kosong jika tidak ada 
                            foto.</small>
                    </div>
